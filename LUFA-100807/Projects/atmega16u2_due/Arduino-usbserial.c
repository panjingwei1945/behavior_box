...
#define cmdread(pin) (!(0 == (PINB & _BV(pin))))

#define cmdwrite(pin, value) (PORTB = (value) ? (PORTB | _BV(pin)) : (PORTB & ~_BV(pin)))

#define txwrite(pin, value) (PORTD = (value) ? (PORTD | _BV(pin)) : (PORTD & ~_BV(pin)))

unsigned char ex_resettime = 0;
